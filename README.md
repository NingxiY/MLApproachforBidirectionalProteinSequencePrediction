...
Spectrum to Sequence inference requires Casanovo 5.1.2. Two components are excluded from this repository due to size and are not downloaded automatically.

### Why they are not in the repo

| Component | Size | Reason excluded |
|-----------|------|-----------------|
| `casanovo/casanovo.ckpt` | ~548 MB | Exceeds practical git limits; model binary |
| `casanovo_env_v5/` | ~1.6 GB | Python virtual environment; machine-specific |

### Step 1 — Get the model checkpoint

Download the checkpoint from the [Casanovo releases page](https://github.com/Noble-Lab/casanovo/releases) and place it at:

```
casanovo/casanovo.ckpt
```

The configuration file (`casanovo/casanovo.yaml`) is already committed in this repo and is compatible with Casanovo 5.1.2.

### Step 2 — Create a Python 3.10 environment

Casanovo 5.1.2 requires Python 3.10. If you do not have it, download it from [python.org](https://www.python.org/downloads/) before continuing.

```bash
# Create the environment
python3.10 -m venv casanovo_env_v5

# Install Casanovo 5.1.2 from source (the PyPI release is an older version)
casanovo_env_v5/bin/pip install "git+https://github.com/Noble-Lab/casanovo.git@v5.1.2"
```

> **macOS only:** The official Python 3.10 installer from python.org installs to
> `~/Library/Frameworks` rather than the system path. The server needs the following
> wrapper script to locate the correct shared libraries at runtime:
>
> ```bash
> cat > casanovo_env_v5/bin/casanovo_run.sh << 'EOF'
> #!/bin/bash
> SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
> FW="$HOME/Library/Frameworks"
> export DYLD_FRAMEWORK_PATH="$FW${DYLD_FRAMEWORK_PATH:+:$DYLD_FRAMEWORK_PATH}"
> export DYLD_LIBRARY_PATH="$FW/Python.framework/Versions/3.10/lib${DYLD_LIBRARY_PATH:+:$DYLD_LIBRARY_PATH}"
> exec "$SCRIPT_DIR/casanovo" "$@"
> EOF
> chmod +x casanovo_env_v5/bin/casanovo_run.sh
> ```
>
> On Linux, skip this step — the plain `casanovo` binary works directly.
> Update `_CASANOVO` in `app.py` to point to `casanovo_env_v5/bin/casanovo` instead.

### Step 3 — Verify inference works

Run a test prediction against the sample file included in the repo:

```bash
casanovo_env_v5/bin/casanovo sequence \
  -m casanovo/casanovo.ckpt \
  -c casanovo/casanovo.yaml \
  -o output \
  casanovo/predicted_spectra_noseq.mgf
```

Casanovo writes results to `output.mztab` in the current directory. If you see PSM rows in that file, the setup is complete.

> If the checkpoint or environment are missing, the Spectrum to Sequence endpoint returns a
> 500 error with a descriptive message. Sequence to Spectrum works independently of Casanovo.
